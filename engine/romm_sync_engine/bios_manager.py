...
class BiosManager:
    ...
    def find_system_directory(self):
        ...
        if platform.system() != 'Windows':
            # RetroDECK is SteamOS/Linux-only; on Windows these paths could
            # only ever be a false-positive match against an unrelated folder.
            possible_dirs = [
        ...
        if platform.system() == 'Windows':
            # Windows RetroArch is portable by default: system/BIOS files live
            # in a "system" folder next to retroarch.exe. Steam/registry/manual
            # install roots all get the same "<root>/system" treatment.
            possible_dirs = [root / 'system' for root in windows_paths.retroarch_root_candidates()] + possible_dirs
            appdata = os.environ.get('APPDATA')
            if appdata:
                possible_dirs.insert(0, Path(appdata) / 'RetroArch' / 'system')
        ...
        # Create RetroDECK bios directory if RetroDECK is detected. Skipped
        # on Windows entirely -- RetroDECK is a SteamOS/Linux distribution
        # and cannot be present there, so this would only ever be a false
        # positive (and, prior to the fix below, always was one).
        if platform.system() != 'Windows':
            retrodeck_bios = Path.home() / 'retrodeck' / 'bios'
            retrodeck_roms = Path.home() / 'retrodeck' / 'roms'
            # NOTE: this previously read `if Path.home() / 'retrodeck' / 'roms'
            # and not ...` -- a Path object is always truthy regardless of
            # whether it exists, so that check always passed and this folder
            # got created unconditionally for every user on every platform,
            # RetroDECK or not. Fixed to actually check for RetroDECK evidence.
            if retrodeck_roms.exists() and not retrodeck_bios.exists():
                try:
